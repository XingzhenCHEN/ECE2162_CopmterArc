...
###Memory
    ...
    if instr.type == 'Ld' or instr.type == 'Sd':
        ...
        type_ = instr.type
        [offset,operand2] = instr.operand2.replace(" ", "").replace(")","").split("(")
        alias = registerAliasTable.get_alias(operand2)
        if 'ROB' in alias:
            if(reorderBuffer.get_value(alias)!= None):
                value_operand2 = reorderBuffer.get_value(alias)
                tag_operand2 = None
            else:
                tag_operand2 = alias
                value_operand2 = None
        else:
            value_operand2 = architecturalRegisterFile.get_value(alias)
            tag_operand2 = None
        ...
        if type_ == 'Ld':
            ...
            if 'ROB' in alias:
                if(reorderBuffer.get_value(alias)!= None):
                    value_operand1 = reorderBuffer.get_value(alias)
                    tag_operand1 = None
                else:
                    tag_operand1 = alias
                    value_operand1 = None
            else:
                value_operand1 = architecturalRegisterFile.get_value(alias)
                tag_operand1 = None
        ...
        memoryUnit.issue_load(instr,idx,type_,target,offset,tag_operand2,value_operand2,tag_operand1,value_operand1)
